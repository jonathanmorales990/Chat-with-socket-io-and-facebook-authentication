...
router.get('/', isLoggedIn , function(req, res, next) {
	...
	User.find( { _id:{ $ne:req.user._id } }, function (err, model) {
		...
		if(err) return res.redirect(303,"/login");
		
		if(!model) return res.render('profile.ejs', {user : req.user});
			        ...
			        // Project pipeline, similar to select
			        {
			             "$project": { 
			                ...
			                "data": 1
			                    ...
			                    if(mensagem) return res.render('profile.ejs', {user : req.user, allUsers : model, notification: mensagem});                  
			});   
		}
	});  
}); 
