  ...
  io.sockets.on( "connection", function( socket ){
      ...
      if (socket.handshake.session.socketID){
        User.findById( socket.handshake.session.passport.user, 'socketId facebook.name facebook.email facebook.photo local.nomecompleto local.nome local.sobrenome local.email local.photo',
          function(err, model) {
            ...
            if (err) socket.emit("error", 'Authentication error');

            if (!model) socket.emit("error", 'Authentication error');
          ...
          User.findByIdAndUpdate(socket.handshake.session.passport.user, { $set: { 'socketId': socket.id} },{safe: true, upsert: true , new:true, fields:'socketId facebook.name facebook.email facebook.photo local.nomecompleto local.nome local.sobrenome local.email local.photo'},
            function(err, model) {
              if (err) socket.emit("error", 'Authentication error');
    ...
    socket.on('mensagem', function (data) {
      ...
      newMensagem.save(function(err, mensagem) {
        if(err) socket.emit('erroMensagem', mensagem);
      ...
      Mensagem.find( { $or: [ { de: socket.handshake.session.passport.user , para:data.id }, { de: data.id, para:socket.handshake.session.passport.user  },]},  
        function(err, mensagem){
            ...
            if (!mensagem.length){
              ...
              Mensagem.update({ de: data.id, para:socket.handshake.session.passport.user },{ readed: true } ,{multi: true} ,
                function(err, mensagem){
                    if(err) socket.emit('requestedMensagemErro', data);
                    if(mensagem) socket.emit('requestedMensagem', data);
              });
            }          
        });
    });
      ...
      Mensagem.aggregate([
          ...
          // Project pipeline, similar to select
          {
               "$project": { 
                ...
                if (err) socket.emit('requestedNotificationsErro', err);

                if(mensagem) socket.emit('requestedNotifications', mensagem);          
          });   
      });
